...
Выведите дерево разбора. Дерево разбора для переменной должно быть размера $1×1$ и содержать эту переменную. Дерево, в корне которого записана операция, с поддеревьями $T_1$ и $T_2$, которые имеют размеры $h_1×w_1$ и $h_2×w_2$ соответственно, должно быть размера $(max[h_1;h_2]+2)×(w_1+w_2+5)$. Подробнее о формате вывода можно узнать, изучив пример вывода (см. ниже). Следует использовать следующие вспомогательные символы: минус ‘-’ (код ASCII 45), точка ‘.’ (код ASCII 46), вертикальная черта ‘|’ (код ASCII 124), квадратные скобки ‘[’ и ‘]’ (коды ASCII 91 and 93).
